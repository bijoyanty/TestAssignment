...
public class GetPage
{
    ...
    public static async Task CreateBrowser(PlaywrightConfig config)
    {
        ...
        _browserInstance = await _browserClass.GetBrowser( config );
    }
    ...
    private async Task<IBrowserContext> CreateBrowserContext()
    {
        ...
        return await _browserContextObject.GetBrowserContext( _browserInstance ?? throw new InvalidOperationException("Browser instance is not created") );
    }
    ...
    public async Task GetNewPage()
    {
        ...
        Page.PageError += (_, exception) => { Console.WriteLine( "Uncaught exception: " + exception ); };
